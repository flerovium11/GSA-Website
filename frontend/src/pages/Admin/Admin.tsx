...
export const Admin: FC<AdminProps> = ({ setUsername }) => {
  ...
  const [blogpostValue, setBlogpostValue] = useState<string>(
    localStorage.getItem("blog-content") ??
      `<h1>Blogtitel</h1>
  ...
  const [ownTitleImageToggle, setOwnTitleImageToggle] =
  ...
  const onSendPassword = (values: any) => {
    if (values["new-password"] !== values["repeat-password"])
      return setInfo("Passwörter sind nicht gleich!", "warning");
    if (values["new-password"] === values["old-password"])
      return setInfo("Es wurde nichts geändert!", "warning");

    setChangePasswordLoading(true);

    backendRequest("update_password", values)
      .then((response) => {
  ...
  const onPostBlog = (values: any) => {
    if (blogpostValue.trim() === "")
      return setInfo(
        "Der Inhalt vom Post darf nicht leer gelassen werden!",
        "warning"
      );
    if (
      values["own-description"] === undefined ||
      values["own-description"].trim() === ""
    )
      return setInfo(
        "Die Blogbeschreibung darf nicht leer gelassen werden!",
        "warning"
      );
  ...
  const continuePostBlog = (values: any) => {
    backendRequest("post_blog", { ...values, content: blogpostValue })
      .then((response) => {
        setInfo(
          `Blog erfolgreich gepostet! Besuche ihn auf ${frontendURL}blog/${response.text}`,
          "success"
        );
  ...
  const onAddAdmin = (values: any) => {
    if (values["new-password"] !== values["repeat-password"])
      return setInfo("Passwörter passen nicht zusammen!", "warning");
    setAddAdminLoading(true);

    backendRequest("add_admin", values)
      .then((response) => {
  ...
  return (
    <>
      <main className="max-w-screen-md min-h-80 mx-auto" id="admin-panel">
        <h1 className="text-2xl pt-7" style={{ marginTop: "100px" }}>
          Admin Panel
        </h1>
        <Tabs
          defaultActiveKey={
            localStorage.getItem("admin-panel-active-tab") ?? "1"
          }
          className="pb-20"
                            ...
                            {usernameValue !== "" && (
                              <Button
                                onClick={() => {
                                  ...
                                  backendRequest("update_username", {
                    ...
                    <Popconfirm
                      ...
                      description={
                        <span>
                          ...
                          <br /> Diese Aktion kann <b>nicht rückgängig</b>{" "}
                          gemacht werden
                        </span>
                      }
                ...
                <>
                  <Form
                    initialValues={{
                      remember: true,
                      "own-description":
                        localStorage.getItem("blog-description") ?? "",
                    }}
                    onFinish={onPostBlog}
                  >
                    <Form.Item>
                    ...
                    <Space direction="vertical">
                      <Form.Item
                        ...
                        rules={[
                          {
                            required: true,
                            message: "Bitte gib den Username ein!",
                          },
                        ]}
                        hasFeedback
                      >
