...
export const Blog: FC = () => {
  ...
  const handleSetReaction = (blogpostID: string, reactionID: string) => {
    ...
    backendRequest("post_reaction", values, null, false)
      .then((response) => {
      ...
      .catch((reason) => {
        setInfo(
          "Verbindung zum Server fehlgeschlagen, versuche es später erneut!",
          "error"
        );
      })
      .finally(() => {
  ...
  const confirmDeleteBlog = (stringID: string) => {
    ...
    backendRequest("delete_post", { string_id: stringID })
      .then((response) => {
        location.reload();
      })
      .catch((reason) => {
  ...
  useEffect(() => {
    ...
    setTimeout(
      () => updateLoadingInfo("Das dauert laenger als erwartet..."),
      2000
    );

    backendRequest("get_posts", {}, null, false)
      .then((response) => {
  ...
  return (
    <main className="blog-container">
        ...
        <>
          ...
          <div className="blog-bottom">
            <div className="author">
              ...
              <span>
                {blogpost.admin_name} am{" "}
                {new Date(blogpost.post_date.replace(/-/g, "/"))
              ...
              {username !== null && (
                <Popconfirm
                  ...
                  description={
                    <span>
                      Bist du sicher, dass du den Post löschen willst? <br />{" "}
                      Diese Aktion kann <b>nicht mehr rückgängig</b> gemacht
                      werden
                    </span>
                  }
            ...
            <div className="reactions">
              {reactions.map((re) => {
                return (
                  <Tooltip
                    key={re.reaction_name}
                    title={re.reaction_name}
                    placement="bottom"
                  >
                    <div
                      className={
                        reaction == re.reaction_id
                          ? "reaction active"
                          : "reaction"
                      }
                      onClick={() =>
                        reaction == re.reaction_id
                          ? handleSetReaction(blogpost.blogpost_id, "")
                          : handleSetReaction(
                    ...
                    >
                      <span className="old-num">
                        {re.reaction_id == initialReaction
                          ? Number(re.num) - 1
                          : Number(re.num)}
                      </span>
                      <span
      ...
      {blogpost === "failed" && (
        <h2 className="m-5 sad">
          Tut uns leid, der Post konnte nicht geladen werden 😢
        </h2>
      )}
      {blogpost === "not found" && (
        <h2 className="m-5 sad">Dieser Post existiert nicht 😭</h2>
      )}
    </main>
  );
};
